...
| :-------------------- | :------------------------------------------------------------------------ | :---------------------------------------------------------------------------------------------------------- | :---------------------------------------------------------------------------------------------------------- |
| Aging granularity     | ❌ binary active / inactive only                                           | ⚠️ 4-gen ring (`MAX_NR_GENS=4`); a full ring triggers a forced `inc_min_seq` oldest-gen drain                 | ✅ 4-gen ring (`MARIE_MAX_NR_GENS=4`); advance skips when the ring is full, no forced drain                  |
...
| Hot-signal harvest    | ❌ rmap-driven `PG_referenced` only; no active walker                      | ⚠️ PTE walker + walker-internal PMD bloom filter + rmap look-around cascading `PG_referenced`                | ✅ PTE walker + rmap-fed PMD bloom filter + rmap look-around updating a saturating tier bit only            |
